...
#  librarian::shelf(tidyverse, here, DBI, odbc, scales, padr, DT, plotly, shiny, ggplot2)
...
# # 
# # update_dt_rlv <- bid_periods %>%
# #   mutate(BID_PERIOD = (as_datetime(paste0(BID_PERIOD, "-25 00:00:00"))-2592000)) %>% 
# #   mutate(BID_PERIOD = paste0(BID_PERIOD, "-25 00:00:00"))
...
#   write_csv(here("shiny_data", "master_history_raw.csv"))
# # 
...
#   write_csv(here("shiny_data", "bid_periods.csv"))
# 
# bases <- c("HNL", "LAX")
...
master_history_raw <- dbGetQuery(db_connection, q_master_history) %>% 
  ...
  filter(!BID_PERIOD == bid_period_remove)
...
bases <- c("HNL", "LAX")
# 
# master_history_raw <- read_csv(here("shiny_data", "master_history_raw.csv")) %>% 
#   mutate(UPDATE_TIME = as.character(UPDATE_DATE),
#          UPDATE_TIME = as.character(UPDATE_TIME)) %>% 
#   filter(!BID_PERIOD == bid_period_remove)
# 
# bid_periods <- read_csv(here("shiny_data", "bid_periods.csv"))%>% 
#   filter(!BID_PERIOD == bid_period_remove)
# 
# bases <- c("HNL", "LAX")
...
plotlyOutput('plot_utl', height = "600px")
...
output$plot_utl <- renderPlotly({
 ...
 utl_p <- ggplot(reserve_utl()) +
    ...
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          ...
          legend.box.spacing = unit(0.35, "cm"),  # Adjust spacing around the legend box
