...
librarian::shelf(tidyverse, here, DBI, odbc, scales, padr, DT, plotly)
...
### Daily RLV Utilization at HNL for the Bid Period: **`r previous_bid_period`**
``` {r}
#| echo: false
#| message: false
#| warning: false
...
hline_data_hnl <- data.frame(
  yintercept = comb_fa_hnl$m_avg_utl)
...
p_fnl <- ggplot(comb_fa_hnl) +
  ...
  geom_text(data = label_data_hnl_used,
            aes(x = DATE, y = (rlv_used-15), label = perc_ut),
  ...
  geom_text(data = comb_fa_hnl, aes(x = max(DATE)+1.6, y = (m_avg_utl+5), label = m_perc_utl), 
            vjust = -1, hjust = 0, color = "black") +
  geom_text(data = tot_rlv_label, aes(x = as.Date(paste0(previous_bid_period, "-15")), y=max(comb_fa_hnl$net_rlv_available),
                                      label = tot))
...
### Daily FA RLV Counts which Recieved an Assignment and then Called out Sick for the Bid Period: **`r previous_bid_period`**

$~$

**Sick Codes:** SOP, 2SK, FLV, FLP, UNA, FLS, FLU, N/S, PER, MGR.  

The final sick code associated with the employee following an assignment determined by the greatest value associated for Update Date and Time column per day per employee.
```{r}
#| echo: false
...
custom_colors <- c(
  colorRampPalette(c("#413691", "#D2058A"))(length(unique(long_data_deg_fa_hnl$sick_type)))
)
...
#### Table of RLV FA that Recieved Assignment and then Called Out Sick
```{r}
#| echo: false
...
### Daily RLV Utilization at LAX for the Bid Period: **`r previous_bid_period`**
``` {r}
#| echo: false
#| message: false
#| warning: false
